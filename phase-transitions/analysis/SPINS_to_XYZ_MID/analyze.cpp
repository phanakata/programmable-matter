...
int avg_spins(int count_index)
{
   ...
   double rBA2=0, rCA2=0, delta_z2=0, dot_product=0, dz, xmid=0, ymid=0, zmid=0;
   int kk=0;	
...
//	 for (int j=edgey+spy; j<NY-spy; j+=spy)
	   {
	     ...
	     xmid = (position[3*(j*NX+i+1)]+position[3*(j*NX+i)])/2.;
	     ymid = (position[3*(j*NX+i+1)+1]+position[3*(j*NX+i)+1])/2.;
	     zmid = (position[3*(j*NX+i+1)+2]+position[3*(j*NX+i)+2])/2.;
	     ...
	     if(dot_product>0)
		       
	       {
                 ...
                 count_index++;
                 xyz[count_index]=xmid; //position[3*(j*NX+i)];
                 count_index++;
                 xyz[count_index]=ymid; //position[3*(j*NX+i)+1];
		 count_index++;
                 xyz[count_index]=zmid; //position[3*(j*NX+i)+2];	
		 count_index++;
