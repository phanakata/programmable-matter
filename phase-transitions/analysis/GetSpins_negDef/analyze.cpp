	 ...
	 for (int j=edgey+spy; j<NY-spy; j+=spy)
	   {
	     ...
	     if (afm==1)
	       {
		 stg = pow(-1, (i-(edgex+spx))/spx+(j-(edgey+spy))/spy);
	       }
	     else
	       {
		 stg=1;
	     ...
	     //DIFFERENT from *posiyive* defects
	     for (int jj=0; jj<3; jj++)
	       {
		 //B relative A
		 rBA[jj]=position[3*(j*NX+i - NX*(spy/2)+spx/2) +jj]-position[3*(j*NX+i - NX*(spy/2)-spx/2)+jj]; 
		 //C relative to A
		 rCA[jj]=(position[3*(j*NX+i + NX*(spy/2)+spx/2)+jj]-position[3*(j*NX+i - NX*(spy/2)-spx/2)+jj])/2.;// use mid point 
		 //defect position relative to A
		 rXA[jj]=position[3*(j*NX+i)+jj]-position[3*(j*NX+i - NX*(spy/2)-spx/2)+jj];
		 
		 //relative pos respect to the mid point 
		 delta_z[jj]=rXA[jj]-rCA[jj];
	       } 
